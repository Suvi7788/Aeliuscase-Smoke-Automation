...
test.describe('Add New Text Message', () => {
    test('Add New Text Message @smoke', async ({ page }) => {
        ...
        await textMessage.addNewTextMessage();
        await textMessage.fillName(textMessageData.name);
        await textMessage.selectName();
        await textMessage.sendMessage();
        await textMessage.verifySuccessMsg();
    })
})